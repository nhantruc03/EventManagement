...
const tablerow = ['Tên người dùng', 'Tên quyền', 'Trạng thái', 'Thao tác']
const keydata = ['userId.name', 'roleId.name', 'isDeleted']
const obj = "credentials"

class listcredentials extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
        ...
        if (this.state.data !== null) {
            return (
                <div className='mt-1'>
                    <div className="row">
                        <div className="col-9">
                            <div className='subject'>Danh sách quyền</div>
                        </div>
                        <div className="col">
                            <Link className="link" to={`/addcredentials`} >
                                <div className="btn btn-createnew">+ Tạo mới</div>
                            </Link>
                        </div>
                    </div>
                    <Search targetParent="userId" target="name" data={this.state.data} getSearchData={(e) => this.getSearchData(e)} />
