...
class ChatRoom extends Component {
    ...
    getData = async () => {
        const temp = await axios.post('/api/chat-scripts/getAll?page=1&limit=15', { scriptId: this.props.roomId })
            .then((res) =>
                res.data.data
            )
        this.setState({
    ...
    sendMessage = async (e) => {
        ...
        var message = {
            ...
            scriptId: this.props.roomId
        }
        
        await axios.post('/api/chat-scripts', message)
            .then((res) => {
    ...
    handleScroll = async (e) => {
        if (e.target.scrollTop === 0) {
            if (!this.state.onLoadMore) {
                ...
                await axios.post(`/api/chat-scripts/getAll?page=${this.state.page + 1}&limit=15`, { scriptId: this.props.roomId })
                    .then((res) => {
