...
class guestTypeView extends Component {
        ...
        return (
            ...
            >
                <Form.Item
                    wrapperCol={{ sm: 24 }}
                    name="name"
                    rules={[{ required: true, message: 'Cần nhập tên loại khách mời' }]}
                >
                    <Input placeholder="Tên loại khách mời..." />
                </Form.Item>
                <br></br>
    ...
    onFinish = async (e) => {
        ...
        console.log(data)
        await trackPromise(
            axios.post('/api/guest-types/', data, {
                headers: {
                    'Authorization': { AUTH }.AUTH
                }
            })
    ...
    deleteClick = async (id) => {
        console.log(id)
        await trackPromise(
            axios.delete('/api/guest-types/' + id, {
                headers: {
                    'Authorization': { AUTH }.AUTH
                }
            })
        ...
        return (
            <div >
                <Row style={{ marginBottom: '20px' }}>
                    ...
                    <Col span={4}>
                        <Button className="back" style={{ float: "right" }} onClick={() => this.setModalVisible(true)}>Thêm loại khách mời</Button>
                    </Col>
                </Row>
