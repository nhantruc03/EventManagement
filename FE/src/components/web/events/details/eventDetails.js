...
class eventDetails extends Component {
  ...
  renderguest = () =>
    this.state.listguesttype.map((e, key) => (
      <TabPane tab={e.name} key={key}>
        <GuestView type={e._id} list={this.state.listguest} />
      </TabPane>
    ));

  renderGroups = () =>
    this.state.listGroups.map((e, key) => (
      <TabPane tab={e.name} key={key}>
  ...
  renderModel = () => {
    return (
      <Tabs defaultActiveKey="1">
        <TabPane tab="Danh sách ban tổ chức" key="1">
  ...
  renderModel2 = () => {
    return (
      <ListGuest
        listguesttype={this.state.listguesttype}
        listguest={this.state.listguest}
        updateGuest={(e) => {
          this.updateguest(e);
        }}
      />
  ...
  renderAvailUser = () => {
    return this.state.data.availUser.map((value, key) => {
      return (
          ...
          <div className="site-layout-background-main">
            <Row style={{ height: "95%" }}>
              <Col sm={24} xl={6} className="event-detail">
                <Title className="event-detail-title" level={4}>
                  Hình thức
                </Title>
                ...
                <div className="flex-container-row">
                  ...
                  <Button
                    ...
                    onClick={() => this.setModal2Visible2(true)}
                  >
                    Chỉnh sửa
                  </Button>
                </div>

                <Tabs defaultActiveKey="1">{this.renderguest()}</Tabs>
              </Col>
              <Col sm={24} xl={10} className="event-detail">
                {/* <Tag className="event-detail-status" style={{ marginTop: '15px' }}>{this.state.status}</Tag> */}
                <Title style={{ color: "#017567", margin: "unset" }} level={1}>
                ...
                <div className="event-detail-time-date-address">
                  ...
                  >
                    <div>
                      <ClockCircleOutlined className="event-detail" />{" "}
                  ...
                  <div style={{ marginTop: "10px" }}>
                    <EnvironmentOutlined className="event-detail" />{" "}
                ...
                <Image
                  ...
                  src={`/api/images/${this.state.data.posterUrl}`}
                  alt="poster"
                ></Image>
                ...
                <div className="flex-container-row">
                  ...
                  <Button className="flex-row-item-right">
                    <Link to={`/addscripts/${this.props.match.params.id}`}>
                      Thêm
                    </Link>
                  </Button>
                </div>
