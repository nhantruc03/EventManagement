...
class eventCard extends Component {
  renderAvailUser = () => {
    return this.props.data.availUser.map((value, key) => {
      return (
          ...
          <Card
            ...
            cover={
              <img
                className="cover"
                alt="example"
                src={`api/images/${this.props.data.posterUrl}`}
              />
            }
          >
            ...
            <Row>
              <img
                ...
                src="/clock.png"
              />{" "}
              {moment(this.props.data.startTime).format("HH:mm")} -{" "}
            ...
            <Row className="eventCardFooter">
              <Col span={20}>
                {this.props.data.tagId.map((value, key) => (
                  <Tag id={value.name} style={{ width: "auto" }} key={key}>
                    {value.name}
                  </Tag>
                ))}
              </Col>
