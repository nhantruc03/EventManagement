...
class edit extends Component {
    ...
    onSubmit = async (e) => {
        await trackPromise(
            Axios.put('/api/credentials/' + this.props.match.params.id, e, {
                headers: {
                    'Authorization': AUTH()
                }
            })
    ...
    async componentDidMount() {
        ...
        const [data] = await trackPromise(Promise.all([
            Axios.get('/api/credentials/' + this.props.match.params.id, {
                headers: {
                    'Authorization': AUTH()
                }
            })
                ...
                <Content style={{ margin: "0 16px" }}>
                    < Row style={{ marginTop: 15, marginLeft: 30, marginRight: 30 }}>
                        <Breadcrumb separator=">">
                            <Breadcrumb.Item >
                                <Link to="/admin/listcredentials">Danh sách</Link>
                            </Breadcrumb.Item>
                            <Breadcrumb.Item>
