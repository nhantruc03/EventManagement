...
export default class Calendarscreen extends Component {
    ...
    async componentDidMount() {
        ...
        const [subActions] = await Promise.all([
            axios.post(`${Url()}/api/sub-actions/getAllWithUserId`, { availUser: obj.id }, {
                headers: {
    ...
    getMarkedDate = () => {
        ...
        this.state.listSubActions.forEach(element => {
            if (!element.status) {
                if (moment(element.endDate).format("YYYY-MM-DD") === this.state.currentDate) {
                    listDate[moment(element.endDate).format("YYYY-MM-DD")] = { marked: true, selected: true }
                } else {
                    listDate[moment(element.endDate).format("YYYY-MM-DD")] = { marked: true }
                }
            }
        })
        return listDate;
        ...
        return (
            <View>
                ...
                <Calendar
                    ...
                    onDayPress={(day) => { this.onSelect(day.dateString) }}
                    // current={'2021-04-27'}
                    theme={{
