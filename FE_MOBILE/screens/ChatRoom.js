...
const client = new WebSocket('ws://192.168.1.27:3001');
class ChatRoom extends Component {
    ...
    getData = async () => {
        const temp = await axios.post(`${Url()}/api/chat-message/getAll?page=1&limit=15`, { roomId: this.props.route.params.id })
            .then((res) =>
                res.data.data
            )
        this.setState({
    ...
    sendMessage = async () => {
        ...
        await axios.post(`${Url()}/api/chat-message`, message)
            .then(res => {
    ...
    renderMessage = (object) => {
        ...
        return (
            <ChatMessage messageClass={msg.userId._id === this.state.currentUser ? 'sent' : 'received'} message={msg} />
        )
    }
    ...
    onRefresh = async () => {
        ...
        await axios.post(`${Url()}/api/chat-message/getAll?page=${this.state.page + 1}&limit=15`, { roomId: this.props.route.params.id })
            .then((res) => {
                    ...
                    <View style={{
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <TextInput
                                ...
                                editable={!this.state.disable}
                            />
                            <TouchableOpacity disabled={!this.state.formValue} onPress={() => this.sendMessage()}>
                                <Image
